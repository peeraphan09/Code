...
size_t webSocketSendFrame(AsyncClient *client, bool final, uint8_t opcode, bool mask, uint8_t *data, size_t len){
  ...
  if(buf == NULL){
    //os_("could not malloc %u bytes for frame header\n", headLen);
    return 0;
  }
  ...
  if(client->add((const char *)buf, headLen) != headLen){
    //os_("error adding %lu header bytes\n", headLen);
    free(buf);
    return 0;
  }
  free(buf);
  ...
  if(len){
    ...
    if(client->add((const char *)data, len) != len){
      //os_("error adding %lu data bytes\n", len);
      return 0;
    }
  }
...
void AsyncWebSocketClient::_onData(void *pbuf, size_t plen){
  ...
  while(plen > 0){
    ...
    if((datalen + _pinfo.index) < _pinfo.len){
      ...
      //os_("frame error: len: %u, index: %llu, total: %llu\n", datalen, _pinfo.index, _pinfo.len);
      //what should we do?
      break;
    }
...
size_t AsyncWebSocketClient::(const char *format, ...) {
  va_list arg;
  va_start(arg, format);
  char* temp = new char[MAX__LEN];
  if(!temp){
    va_end(arg);
    return 0;
  }
  char* buffer = temp;
  size_t len = vsn(temp, MAX__LEN, format, arg);
  va_end(arg);
...
#ifndef ESP32
size_t AsyncWebSocketClient::_P(PGM_P formatP, ...) {
  va_list arg;
  ...
  char* buffer = temp;
  size_t len = vsn_P(temp, MAX__LEN, formatP, arg);
  va_end(arg);
  ...
  if (len > (MAX__LEN - 1)) {
    ...
    va_start(arg, formatP);
    vsn_P(buffer, len + 1, formatP, arg);
    va_end(arg);
  }
  text(buffer, len);
  if (buffer != temp) {
...
size_t AsyncWebSocket::(uint32_t id, const char *format, ...){
  AsyncWebSocketClient * c = client(id);
  if(c){
    va_list arg;
    va_start(arg, format);
    size_t len = c->(format, arg);
    va_end(arg);
    return len;
  }
  return 0;
...
size_t AsyncWebSocket::All(const char *format, ...) {
  va_list arg;
  char* temp = new char[MAX__LEN];
  if(!temp){
    return 0;
